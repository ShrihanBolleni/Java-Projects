/* This program is a simple word guessing game where the user tries to guess a randomly chosen four-letter word from a list. It displays the list of words, prompts the user to guess, and provides feedback on the correctness of the guess. */
...
public class WordGuessingGame
{
	...
	public static void guesses()
	{
        ...
        boolean a=true;
        int gs=4;
		// Loop to handle guesses and provide feedback
        while(a&&gs>0)
        {
            if(guess.equals(answer))
            {
                System.out.println("4 characters are correct.\n");
            ...
            else
            {
                ...
                int c=0;
                int g=0;
				// Loop to compare guess with the answer
                for(int i=0;i<answer.length();i++)
                {
                	y=true;
                	g=0;
                    while(y&&g<guess.length())
                    {
                        if(answer.substring(i,i+1).equals(guess.substring(g,g+1)))
                        {
                            c++;
                            y=false;
                        }
                ...
                gs--;
                System.out.println(c+" characters are correct.");
                System.out.println("You have "+gs+" guesses remaining.");
                if(gs!=0)
                {
                	System.out.println("\n\nGuess the word from the list above.");
                	guess=input.next();
